--  Standalone test suite for B_Star (main program).

pragma Ada_2022;

with Ada.Text_IO;
with B_Star; use B_Star;
...
begin
   Ada.Text_IO.Put_Line ("B* test suite");
   ...
   begin
      ...
      Clear (G);
      L := Add_Leaf (G, 1_000_000);
      Set_Root (G, L);
      Check_Both (G, 1_000_000, "leaf large");
   end;
   ...
   ---------------------------------------------------------------------
   Section ("5. Classic max(min(3,5), min(2,9)) = 3");
   ---------------------------------------------------------------------
   declare
      G : Game_Tree;
   ...
   begin
      ...
      Set_Root (G, Root);
      Check_Both (G, 3, "classic shallow");
   end;
   ...
   declare
      G : Game_Tree;
      A, B, C, D, E, F, F2, M1, M2, M3, R : Valid_Node;
   begin
      Clear (G);
      A := Add_Leaf (G, 1);
      B := Add_Leaf (G, 2);
   ...
   begin
      ...
      begin
         ...
         --  no root
         U := Minimax (G);
         Check (False, "Minimax no root (no exception)");
      exception
         when Invalid_Argument =>
            Check (True, "Minimax no root");
      end;
      ...
      begin
         ...
         L := Add_Leaf (G, 1);
         K := Kind_Of (G, 2);
         Check (False, "Kind_Of unknown (no exception)");
      exception
         when Invalid_Argument =>
            Check (True, "Kind_Of unknown");
      end;
      ...
      begin
         ...
         L := Add_Leaf (G, 1);
         U := Leaf_Value (G, 2);
         Check (False, "Leaf_Value unknown (no exception)");
      exception
         when Invalid_Argument =>
            Check (True, "Leaf_Value unknown");
      end;

      begin
         Clear (G);
         L := Add_Leaf (G, 1);
         N := Add_Internal (G, Max_Node, [99]);
         Check (False, "unknown child index (no exception)");
      exception
         when Invalid_Argument =>
            Check (True, "unknown child index");
      end;

      pragma Unreferenced (U, N, K);
   end;

   ---------------------------------------------------------------------
   Section ("16. More fixed max-of-mins fixtures");
   ---------------------------------------------------------------------
   declare
      G : Game_Tree;
      type Int_Vec is array (Positive range <>) of Integer;
   ...
   begin
      Max_Of_Pair_Mins ([3, 5, 2, 9], 3, "fix classic");
   ...
   ---------------------------------------------------------------------
   Section ("17. Deeper Min-root tree");
   ---------------------------------------------------------------------
   declare
      G : Game_Tree;
   ...
   ---------------------------------------------------------------------
   Section ("19. Equal-merit branches");
   ---------------------------------------------------------------------
   declare
      G : Game_Tree;
   ...
   ---------------------------------------------------------------------
   Section ("21. Prove-best / disprove-rest shaped trees");
   ---------------------------------------------------------------------
   declare
      G : Game_Tree;
      --  Leader clearly better after expansion: max(min(9,8), min(1,2), min(0,3))
      --  = max(8,1,0) = 8
      A, B, C, D, E, F, M1, M2, M3, R : Valid_Node;
   begin
      Clear (G);
      ...
      R := Add_Internal (G, Max_Node, [M1, M2, M3]);
      Set_Root (G, R);
      Check_Both (G, 8, "strong leader 8");

      --  Runner-up needs disproving: max(min(5,6), min(5,7), min(4,10))
      --  = max(5,5,4) = 5
      Clear (G);
      A := Add_Leaf (G, 5);
      B := Add_Leaf (G, 6);
      ...
      R := Add_Internal (G, Max_Node, [M1, M2, M3]);
      Set_Root (G, R);
      Check_Both (G, 5, "close rivals 5");

      --  Min root with optimistic rivals: min(max(2,3), max(4,1), max(0,5))
      --  = min(3,4,5) = 3
      Clear (G);
      A := Add_Leaf (G, 2);
      B := Add_Leaf (G, 3);
      ...
      declare
         X1, X2, X3 : Valid_Node;
      begin
         X1 := Add_Internal (G, Max_Node, [A, B]);
         X2 := Add_Internal (G, Max_Node, [C, D]);
         X3 := Add_Internal (G, Max_Node, [E, F]);
         R := Add_Internal (G, Min_Node, [X1, X2, X3]);
         Set_Root (G, R);
         Check_Both (G, 3, "min root rivals 3");
      end;
   end;

   ---------------------------------------------------------------------
   Section ("22. Nested bushy tree");
   ---------------------------------------------------------------------
   declare
      G : Game_Tree;
      --  max(
      --    min(max(1,2), max(0,3)),   -- min(2,3)=2
      --    min(max(4,0), max(5,1)),   -- min(4,5)=4
      --    min(max(2,2), max(9,-1))   -- min(2,9)=2
      --  ) = 4
      procedure Leaf2 (V1, V2 : Integer; Out_N : out Valid_Node) is
         A, B : Valid_Node;
      begin
         A := Add_Leaf (G, V1);
         B := Add_Leaf (G, V2);
         Out_N := Add_Internal (G, Max_Node, [A, B]);
      end Leaf2;
      X1, X2, X3, X4, X5, X6, N1, N2, N3, R : Valid_Node;
   begin
      Clear (G);
      Leaf2 (1, 2, X1);
      Leaf2 (0, 3, X2);
      ...
      N1 := Add_Internal (G, Min_Node, [X1, X2]);
      N2 := Add_Internal (G, Min_Node, [X3, X4]);
      N3 := Add_Internal (G, Min_Node, [X5, X6]);
      R := Add_Internal (G, Max_Node, [N1, N2, N3]);
      Set_Root (G, R);
      Check_Both (G, 4, "bushy nested = 4");
   end;
